...
@section('header', 'Cập nhật Người dùng')
    ...
    <div class="col-md-8 mx-auto">
        <div class="card card-warning card-outline mb-4">
            <div class="card-header">
                <div class="card-title">Chỉnh sửa thông tin: {{ $user->name }}</div>
            </div>
            <!-- /.card-header -->
            <!-- form start -->
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <select class="form-select @error('role') is-invalid @enderror" id="role" name="role" required>
                                <option value="Admin tổng" {{ old('role', $user->role) == 'Admin tổng' ? 'selected' : '' }}>Admin tổng</option>
                                <option value="Admin kho" {{ old('role', $user->role) == 'Admin kho' ? 'selected' : '' }}>Admin kho</option>
                                <option value="Nhân viên kho" {{ old('role', $user->role) == 'Nhân viên kho' ? 'selected' : '' }}>Nhân viên kho</option>
                            </select>
                            @error('role')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select class="form-select @error('warehouse_id') is-invalid @enderror" id="warehouse_id" name="warehouse_id">
                                ...
                                <option value="{{ $wh->id }}" {{ old('warehouse_id', $user->warehouse_id) == $wh->id ? 'selected' : '' }}>{{ $wh->name }}</option>
                                @endforeach
                            </select>
